...
namespace IBMS.Web.API.Controllers
{
    ...
    public class PremiumController : ApiController
    {
        ...
        [HttpPost()]
        [ActionName("SavePremium")]
        public IHttpActionResult SavePremium([FromBody]JObject data)
        {
            try
            {
               
                var clientObj = data.SelectToken("ClientObj").ToObject<PremiumVM>();      
              
                bool status = manageUtility.SavePremium(clientObj);
        ...
        [HttpPost()]
        [ActionName("getAllPremum")]
        public IHttpActionResult getAllPremum([FromBody]JObject data)
        {
            try
            {
               int businessUnitID = !string.IsNullOrEmpty(data.SelectToken("businessUnitID").Value<string>()) ? Convert.ToInt32(data.SelectToken("businessUnitID").Value<string>()) : 0;
                var agentList = manageUtility.GetPremium(businessUnitID);
                return Json(new
                {
                    status = true,
                    data = agentList
        ...
        public IHttpActionResult GetRequestByID([FromBody]JObject data)
        {
            try
            {
                int agentID = !string.IsNullOrEmpty(data.SelectToken("ClientRequestHeaderID").Value<string>()) ? Convert.ToInt32(data.SelectToken("ClientRequestHeaderID").Value<string>()) : 0;
                var agent = manageUtility.GetPremiumByID(agentID);
                return Json(new
                {
                    status = true,
                    data = agent
