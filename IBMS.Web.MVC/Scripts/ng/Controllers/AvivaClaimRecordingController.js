...
ibmsApp.controller("AvivaClaimRecordingController", function ($scope, $http, $rootScope, AvivaClaimRecordingService, $location, AuthService, filterFilter, $modal) {
    ...
    $scope.editClaimRecording = function (ReferenceNo) {
        ...
        $scope.showLoader = true;
        AvivaClaimRecordingService.editClaimRecording(ReferenceNo).then(function (results) {
            $scope.showLoader = false;
            if (results.status === true) {
                $scope.isViewMode = false;
            ...
            buttons: [
                    {
                        addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function ($noty) {
                            ...
                            AvivaClaimRecordingService.updateClaimRecording($scope.Admission, $scope.currentUser.UserID).then(function (results) {
                                $scope.showLoader = false;
            ...
            buttons: [
                    {
                        addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function ($noty) {
                          ...
                          //  $scope.Admission.PaymentAdviceReceviedDate = $scope.getFormattedDate($scope.Admission.PaymentAdviceReceviedDate);
                            ...
                            AvivaClaimRecordingService.saveClaimRecording($scope.Admission, $scope.currentUser.UserID).then(function (results) {
                                $scope.showLoader = false;
                                //alert(angular.toJson(results));
                                if (results.status === true) {
                                    noty({
