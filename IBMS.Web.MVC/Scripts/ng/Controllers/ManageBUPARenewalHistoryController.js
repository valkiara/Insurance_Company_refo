...
ibmsApp.controller("ManageBUPARenewalHistoryController", function ($scope, $http, $rootScope, ManageTransactionsService, $location, AuthService, filterFilter, $modal) {
    ...
    $scope.loadClientRequestsByBUID = function (businessUnitID) {
        $scope.showLoader = true;
        ManageTransactionsService.getAllHistoryClientRequestsByBUID(businessUnitID).then(function (results) {
            $scope.showLoader = false;
            if (results.status === true) {
                $scope.availableHistory = results.data;
                ...
                //    $scope.availableHistory.isManagePayment = true;
                //else
                //    $scope.availableHistory.isManagePayment = false;


                $scope.data = angular.copy($scope.availableHistory);
                $scope.viewby = "10";
    ...
    $scope.searchTextChange = function (searchText) {
        $scope.data = filterFilter($scope.availableHistory, searchText);
        $scope.viewby = "10";
        $scope.totalItems = $scope.data.length;
        $scope.currentPage = 1;
        $scope.itemsPerPage = $scope.viewby;
    ...
    $scope.loadClientByID = function (clientID, Year,FrequncyID, FrequncyDID) {
        $scope.showLoader = true;
        // alert("ok");
        ManageTransactionsService.getClientHistrotyByID(clientID, Year,FrequncyID, FrequncyDID).then(function (results) {
            $scope.showLoader = false;
            if (results.status === true) {
                ...
                if ($scope.cusObj.DeductionDetails.length == 0) {
                    ...
                    if ($scope.cusObj.PremiumAccept == '1') {
                        ...
                        $scope.cusObj.DeductionDetails.push({ PremiumHolder: $scope.cusObj.ClientOtherName,MI:$scope.cusObj.MembershipID, Premium: "", LoadingRate: "", DeductionRate: "", NetPremium: "" });
                    ...
                    for (var i = 0; i < $scope.cusObj.FamilyDetails.length; i++) {
                        ...
                        if ($scope.cusObj.FamilyDetails[i].MemberName != "")
                            $scope.cusObj.DeductionDetails.push({ PremiumHolder: $scope.cusObj.FamilyDetails[i].MemberName,MI:$scope.cusObj.FamilyDetails[i].MembershipID,Premium: "", LoadingRate: "", DeductionRate: "", NetPremium: "" });
                            ...
                            $scopeChild.updateCustomerDetails = function () {
                                ...
                                for (var i = 0; i < $scopeChild.FamilyDetails.length; i++) {
                                    ...
                                    if ($scope.cusObj.DeductionDetails !== undefined && $scope.cusObj.DeductionDetails !== "" && $scope.cusObj.DeductionDetails !== null) {
                                        var obj = $scope.cusObj.DeductionDetails.filter(p => p.PremiumHolder == $scopeChild.FamilyDetails[i].MemberName)
                                        ...
                                        for (var j = 0; j < $scopeChild.FamilyDetails[i].GroupMemberDetails.length; j++) {
                                            ...
                                            var objGrpMember = $scope.cusObj.DeductionDetails.filter(p => p.PremiumHolder == $scopeChild.FamilyDetails[i].GroupMemberDetails[j].MemberName)
                                            ...
                                            $scopeSubChild.addInsCompanyScopeDetails = function () {
                                                ...
                                                if ($scopeSubChild.quotationDetailsInsCompanyScopeTemp === undefined || $scopeSubChild.quotationDetailsInsCompanyScopeTemp === null || $scopeSubChild.quotationDetailsInsCompanyScopeTemp === "") {
                                                    ...
                                                    if ($scope.cusObj.FamilyDetails[idx].GroupMemberDetails.length === 0) {
                                                        ...
                                                        $scopeSubChild.quotationDetailsInsCompanyScopeTemp.push({ TitleID: "", MemberName: "", MemberOtherName: "", MemberDOB: "", JoinDate: "", MembershipID: "", ContactNo: "", RelationShipID: "", GenderID: "", IsActive: "" });
                                                    }
...
parseInt(results.data.PaiedAmount.toString()) == 0) {
            ...
            buttons: [
                    {
                        addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function ($noty) {
                            ...
                            ManageTransactionsService.saveBUPARenewelClientRequest($scope.isCustomerUpdated, $scope.isCustomerAdded, $scope.cusObj, $scope.cusReqObj, $scope.currentUser.UserID).then(function (results) {
                                $scope.showLoader = false;
                                //  alert(angular.toJson(results));
                                if (results.status === true) {
                                    noty({
            ...
            buttons: [
                    {
                        addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function ($noty) {
                            ...
                            ManageTransactionsService.updateRenwelClientRequest(true, $scope.isCustomerAdded, $scope.cusObj, $scope.cusReqObj, $scope.currentUser.UserID).then(function (results) {
                                $scope.showLoader = false;
                                //alert(angular.toJson(results));
                                if (results.status === true) {
                                    noty({
    ...
    $scope.viewClientRequest = function (clientReqHeaderID, clientID, Year, FrequncyID, FrequncyDID) {
        $scope.activateNewClientRequestTab();

        $scope.showLoader = true;
        ManageTransactionsService.getClientHistoryRequestByID(clientReqHeaderID, clientID, Year, FrequncyID, FrequncyDID).then(function (results) {
            $scope.showLoader = false;
            if (results.status === true) {
                ...
                $scope.cusReqObj = results.data;
                $scope.cusReqObj.PartnerID = $scope.cusReqObj.PartnerID + "";
                $scope.loadClientByID(clientID, Year, FrequncyID, FrequncyDID);
                if ($scope.cusReqObj.PaymentDetails.length > 0) {
                    $scope.cusReqObj.PaymentAmount = $scope.cusReqObj.PaymentDetails[0].PaymentAmount;
                }
                else {
                ...
                $scope.loadClientByID(clientID, Year, FrequncyID, FrequncyDID);
                $scope.LoadFrequncyData(FrequncyID);
