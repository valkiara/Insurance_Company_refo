...
ibmsApp.factory('AgentService', function ($http, $rootScope) {
    ...
    var getAgentCommission = function (fDate, tDate, cType) {
        try {
            var params = $.param({ "rootObj": { "fromDate": fDate, "toDate": tDate, "type": cType } });
            return $http.post($rootScope.serviceURL + 'api/Agent/GetAgentCommission', params, config2).then(function (results) {
